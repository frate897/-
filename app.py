...
try:
    ...
    if res.data:
        ...
        # ==================== 💡 核心新增：生成并下载 Excel 表格 ====================
        # 把数据整理成好看的表格格式
        df_export = df_raw.rename(columns={"date": "日期", "type": "类型", "amount": "金额(元)", "reason": "原因_备注"})
        df_export = df_export[["日期", "类型", "金额(元)", "原因_备注"]] # 只保留需要的列
        
        # 在内存中把数据打包成标准的 Excel 文件
        buffer = io.BytesIO()
        with pd.ExcelWriter(buffer, engine='openpyxl') as writer:
            df_export.to_excel(writer, index=False, sheet_name='餐馆财务明细')
        buffer.seek(0)
        
        # 创建一个显眼的大按钮
        st.download_button(
            label="📥 导出全部账目为 Excel 表格 (.xlsx)",
            data=buffer,
            file_name=f"餐馆财务报表_{datetime.now().strftime('%Y%m%d')}.xlsx",
        ...
        # 用循环的方式把账目一条条显示出来，并在每条账目后面配一个删除按钮
